			...
			React.createElement(
				...
				data.showGrid ? data.xAxisPoints.map(function (item, i) {
					return React.createElement(
						...
						React.createElement('line', { x1: item + "%", y1: data.yAxisMaximum+"%", x2: item + "%", y2: data.yAxisMinimum+"%" })
					);
				}) : "",
				'/* y-axis grid lines */',
				data.showGrid ? data.yAxisPoints.map(function (item, i) {
					return React.createElement(
						'g',
						{ key: i, stroke: data.chartStyle.gridColor },
						React.createElement('line', { x1: data.xAxisMinimum+"%", y1: item + "%", x2: data.xAxisMaximum+"%", y2: item + "%" })
					);
				}) : "",
				'/* axes lines */',
				React.createElement(
					...
					React.createElement('line', { x1: data.xAxisMinimum+"%", y1: data.yAxisMaximum+"%", x2: data.xAxisMinimum+"%", y2: data.yAxisMinimum+"%" }),
					React.createElement('line', { x1: data.xAxisMinimum+"%", y1: (data.yAxisMaximum+0.1)+"%", x2: data.xAxisMaximum+"%", y2: (data.yAxisMaximum+0.1)+"%" })
				),
				'/* y-axis text */',
					...
					return React.createElement(
						...
						React.createElement(
							...
							{ alignmentBaseline: 'middle', textAnchor: 'end',
								x: (data.xAxisMinimum-data.yAxisTextGap)+"%", y: data.yAxisPoints[i] + "%" },
							yAxis
						)
					);
				}),
					...
					return React.createElement(
						...
						React.createElement(
							...
							{ alignmentBaseline: 'before-edge', textAnchor: 'middle',
								y: (data.yAxisMaximum+data.xAxisTextGap)+"%", x: data.xAxisPoints[i] + "%" },
							yAxis
						)
					);
				}),
